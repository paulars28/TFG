...
result_files = {
    "64-32": "/home/gtav-tft/Desktop/paula/eval/COMP_TAMAÑOSRED/64-32/eval/results.txt",
    "32-16": "/home/gtav-tft/Desktop/paula/eval/COMP_TAMAÑOSRED/32-16/eval/results.txt",
    "16-8": "/home/gtav-tft/Desktop/paula/eval/COMP_TAMAÑOSRED/16-8/eval/results.txt",
    "10-5": "/home/gtav-tft/Desktop/paula/eval/COMP_TAMAÑOSRED/10-5/eval/results.txt",
...
def parse_metrics(filepath):
    ...
    rf = re.findall(r"RandomForest entrenado en TVAE - Accuracy: ([0-9.]+), F1: ([0-9.]+)", content)
    rf_kan = re.findall(r"RandomForest entrenado en KAN - Accuracy: ([0-9.]+), F1: ([0-9.]+)", content)
    gb = re.findall(r"GradientBoosting entrenado en TVAE - Accuracy: ([0-9.]+), F1: ([0-9.]+)", content)
    gb_kan = re.findall(r"GradientBoosting entrenado en KAN - Accuracy: ([0-9.]+), F1: ([0-9.]+)", content)

    if rf: metrics["RF_TVAE_Acc"], metrics["RF_TVAE_F1"] = map(float, rf[0])
    if rf_kan: metrics["RF_KAN_Acc"], metrics["RF_KAN_F1"] = map(float, rf_kan[0])
    if gb: metrics["GB_TVAE_Acc"], metrics["GB_TVAE_F1"] = map(float, gb[0])
    if gb_kan: metrics["GB_KAN_Acc"], metrics["GB_KAN_F1"] = map(float, gb_kan[0])

    # Likelihood y MMD
    ll = re.findall(r"TVAE - Lsyn: (-?[0-9.]+), Ltest: (-?[0-9.]+)", content)
    ll_kan = re.findall(r"KAN\s+- Lsyn: (-?[0-9.]+), Ltest: (-?[0-9.]+)", content)
    mmd = re.findall(r"TVAE - MMD: (-?[0-9.]+)", content)
    mmd_kan = re.findall(r"KAN\s+- MMD: (-?[0-9.]+)", content)


    if ll: metrics["LL_TVAE_syn"], metrics["LL_TVAE_test"] = map(float, ll[0])
    if ll_kan: metrics["LL_KAN_syn"], metrics["LL_KAN_test"] = map(float, ll_kan[0])
    if mmd: metrics["MMD_TVAE"] = float(mmd[0])
    ...
    mae_corr = re.findall(
        r"=== (TVAE|KAN) ===\s+MAE de la matriz de correlación \(Pearson\): ([0-9.]+)", 
        content
    )
...
def parse_tstr_trts_metrics_from_text(text):
    ...
    for i in range(1, len(blocks), 2):
        ...
        evals = re.findall(r"(TSTR_TVAE|TSTR_KAN|TRTS_TVAE|TRTS_KAN): ([^\n]+)", block_body)
...
def plot_mae_variables(df, images_file):    
    ...
    mae_pattern = re.compile(r"MAE_(.+)_TVAE")
    variables = sorted({
    ...
    for idx, row in df.iterrows():
        for var in variables:
            col_tvae = f"MAE_{var}_TVAE"
            col_kan  = f"MAE_{var}_KAN"
            if col_tvae in row and col_kan in row:
                data.append({"Arquitectura": str(idx), "Variable": var, "Modelo": "TVAE", "MAE": row[col_tvae]})
...
def plot_metrics_accf1(df, images_file):
    ...
    ax.bar(x - width/2, df['RF_TVAE_Acc'], width, label='TVAE', color=COLORES_MODELO["TVAE"])
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.bar(x - width/2, df['RF_TVAE_F1'], width, label='TVAE', color=COLORES_MODELO["TVAE"])
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.bar(x - width/2, df['GB_TVAE_Acc'], width, label='TVAE', color=COLORES_MODELO["TVAE"])
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.bar(x - width/2, df['GB_TVAE_F1'], width, label='TVAE', color=COLORES_MODELO["TVAE"])
...
def compare_MAE(df, images_file):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.bar(x - width/2, df['MAE_Corr_TVAE'], width, label='TVAE', color=COLORES_MODELO["TVAE"])
...
def plot_importance(df, images_file):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        tvae_cols = [col for col in row.index if col.startswith("Imp_TVAE_")]
        ...
        imp_tvae = [row[f"Imp_TVAE_{v}"] for v in variables]
        ...
        ax.bar(x,         imp_tvae, width, label="TVAE", color=pastel_colors["TVAE"])
...
def plot_mmd(df, images_file):
    ...
    ax.bar(x - width/2, df['MMD_TVAE'], width, label='TVAE', color=COLORES_MODELO["TVAE"])
...
def plot_all_mae_variables(df, images_file):
    # Expresión regular para detectar columnas tipo MAE_xxx_TVAE
    mae_pattern = re.compile(r"MAE_(.+)_TVAE")

    # Extraer variables comunes a ambos modelos (TVAE y KAN)
    variables = {
        mae_pattern.match(col).group(1)
        for col in df.columns
    ...
    if 'MAE_Corr_TVAE' in df.columns and 'MAE_Corr_KAN' in df.columns:
        variables.add('Corr')
    ...
    for idx, row in df.iterrows():
        for var in variables:
            col_tvae = f"MAE_{var}_TVAE"
            col_kan  = f"MAE_{var}_KAN"
            if col_tvae in df.columns and col_kan in df.columns:
                nombre_variable = "HeartDisease" if var == "Corr" else var
                arquitectura = str(row["Architecture"]) if "Architecture" in row else str(idx)
                data.append({"Arquitectura": arquitectura, "Variable": nombre_variable, "Modelo": "TVAE", "MAE": row[col_tvae]})
...
def plot_parallel_trts_tstr(df, images_file, modelo_clasificador):
    ...
    # Paleta de colores por arquitectura
    ...
    for evaluacion in ['TSTR', 'TRTS']:
        ...
        # Ajustar cada eje a sus propios valores
        yaxis_title = "Valor"
        fig.update_layout(
        ...
        # NO fijamos yaxis global: se mantiene cada valor real visible
        fig.update_yaxes(autorange=True)

        # Guardar
        html_filename = os.path.join(images_file, f"grafico_paralelo_{modelo_clasificador}_{evaluacion}.html")
        fig.write_html(html_filename)

        png_filename = os.path.join(images_file, f"grafico_paralelo_{modelo_clasificador}_{evaluacion}.png")
